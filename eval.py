...
model_en.load_state_dict(torch.load('pretrained/model_en.pkl'), strict=False)
model_de.load_state_dict(torch.load('pretrained/model_de.pkl'), strict=False)
model_class.load_state_dict(torch.load('pretrained/model_class.pkl'), strict=False)
...
testImgPath = 'perspective_test2.jpg'
saveFlowPath = 'imgs/'
...
disimgs = io.imread(testImgPath)
from PIL import Image
disimgs = Image.fromarray(disimgs)
disimgs = transform(disimgs)
...
distorted_img = disimgs.data.cpu().squeeze().view([256, 256, 3]).numpy()

resImg, resMsk = rectification(distorted_img, flow)